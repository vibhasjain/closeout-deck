...
(() => {
  ...
  // Hover pauses only over the copy and the controls: the hero fills the screen, so pausing on the whole
  // section would mean a desktop visitor's resting cursor stops the rotation for good.
  const hoverZones=[...carousel.querySelectorAll('.over-copy, .carousel-arrow, .carousel-controls')];
  const overZone=()=>hoverZones.some(z=>z.matches(':hover'));
  let hovered=matchMedia('(hover: hover)').matches&&overZone();
  ...
  hoverZones.forEach(z=>{
    z.addEventListener('pointerenter',e=>{ if(e.pointerType!=='touch'){ hovered=true; sync(); } });
    z.addEventListener('pointerleave',e=>{ if(e.pointerType!=='touch'){ hovered=overZone(); sync(); } });
  });
