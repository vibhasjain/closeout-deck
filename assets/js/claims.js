/* Documentation and swap points only; the HTML intentionally carries its copy inline. Earlier entries are in git history. */
(() => {
  const defaultSource = 'HyperTrack; pending public citation';
  const entry = (value, status, source = defaultSource) => ({ value, status, source });
