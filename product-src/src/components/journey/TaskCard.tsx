...
// eslint-disable-next-line react-refresh/only-export-components
export function taskProgress(cycle?: CyclePayload, row?: CycleSummary, empty = false) {
  const counts = cycle?.counts ?? row?.counts ?? { set1: 0, set2: 0, set3: 0 }
  return {
    // Before any time entries arrive nothing is running; the list may still advertise an older run while its detail loads.
    status: cycle?.runAt ? 'Done' as const : empty ? 'No time entries yet' as const : 'Running' as const,
...
export function TaskCard({ cycleId, onAnswer }: { cycleId: string; onAnswer?(answer: string): void }) {
  const { cycle, row, empty, error } = useJourneyCycle(cycleId)
  const task = taskProgress(cycle, row, empty)
  ...
  return <section className="journey-task" aria-label={`Closeout · ${label}`}>
    <div className="journey-task-title"><h3>Closeout · {label}</h3><span role="status"><Tag className="journey-task-status">{task.status === 'Done' ? <Check size={12} aria-hidden /> : task.status === 'Running' && <Spinner />}{task.status}</Tag></span></div>
