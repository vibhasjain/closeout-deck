...
export function PayRuns({ onNavigate }: { onNavigate?: () => void } = {}) {
  ...
  return <section id={id} className={`sidebar-pay-runs${open ? ' pay-runs-open' : ''}`} aria-label="Pay runs">
    ...
    {error ? <div className="pay-runs-load-state" role="alert"><p>Pay runs could not be loaded.</p><button type="button" className="btn" onClick={() => void invalidate()}>Retry</button></div> : loaded === false ? (hasWarmResponseCache() ? null : <SkeletonRegion variant="rail" />) : <ClusterList kind="cycles" selected={cycle.id} onSelect={selectCycle}
