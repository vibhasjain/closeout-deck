...
describe('call card rendering', () => {
  ...
  it('fetches once on open, shows loading and caches the validated turns through reopening', async () => {
    ...
    const loading = renderToStaticMarkup(renderCard({ card }))
    expect(loading).toContain('data-skeleton="conversation"')
    expect(loading).toContain('aria-busy="true"')
    expect(loading.replace(/<span class="sr-only[^"]*">.*?<\/span>/g, '').replace(/<[^>]+>/g, ' ')).not.toMatch(/loading|reading your|fetching/i)
