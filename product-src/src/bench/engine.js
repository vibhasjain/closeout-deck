...
const fmtT = m => { const mm = ((m % 1440) + 1440) % 1440, hh = Math.floor(mm / 60); return `${hh % 12 || 12}:${String(mm % 60).padStart(2, '0')} ${hh < 12 ? 'AM' : 'PM'}`; };
const fmtH = m => (m / 60).toFixed(2);
// Durations for people to read: 137 → "2h 17m".
const fmtHM = m => { const t = Math.round(m), h = Math.floor(t / 60), r = t % 60; return h && r ? `${h}h ${r}m` : h ? `${h}h` : `${r}m`; };
...
const FACILITIES = {
  ...
  sutter:   { name: 'Pacific Cold Storage', city: 'Ontario', state: 'CA', vertical: 'warehouse', lat: 34.06, lng: -117.65, geofence: true,  badge: false, autoDeduct: false, minWage: 16.50 },
  ...
  lonestar: { name: 'Lonestar Packaging', city: 'Dallas', state: 'TX', vertical: 'warehouse', lat: 32.78, lng: -96.80, geofence: true, badge: false, autoDeduct: false, minWage: 7.25 },
  stmarks:  { name: 'Harbor Point Hotel', city: 'Oakland',    state: 'CA', vertical: 'hospitality', lat: 37.80, lng: -122.25, geofence: true,  badge: false, autoDeduct: false, minWage: 16.50 },
};
const distMi = (a, b) => {
...
const FEATURES = [
  ...
  ['scheduled_min', 'Scheduled length, minutes', s => s.sched ? s.sched[1] - s.sched[0] : null],
  ...
  ['gap_since_prev_min', 'Rest gap since the previous clock-out (same worker), minutes', (s, c) => c.gapSincePrev(s)],
  ['overlap_shift', 'Another time entry by the same worker overlapping in time', (s, c) => c.overlap(s)],
  ['travel_speed_mph', 'Implied speed from previous site to this clock-in', (s, c) => c.travelSpeed(s)],
  ['exact_streak', 'Consecutive time entries with identical punched duration', (s, c) => c.exactStreak(s)],
  ['edited_after_approval', 'Time entry changed after supervisor approval', s => !!s.editedAfterApproval],
  ...
  ['orientation', 'Time entry is for training or orientation', s => !!s.orientation],
...
const RULES = [
  {
    ...
    sentence: 'Two clock-ins within a few minutes are one time entry, not two',
    source: { doc: 'Ops heuristic', cite: 'Duplicate punches on shared kiosks' },
    scope: () => true,
    params: { window_min: p(10, 2, 30, 1, 'min', 'Merge window') },
    evaluate(s, ctx, P) {
      const g = ctx.dupGap(s);
      if (g == null) return [{ status: 'pass', note: `1 clock-in found` }];
      return [{ status: 'applied', note: `Clocked in twice, ${g} minutes apart. We kept one clock-in so the day isn't paid twice.`, overNaive: true }];
    },
  },
  {
    ...
    evaluate(s, ctx) {
      ...
      const rows = [{ status: 'flag', note: 'No clock-out on the time entry' }];
      if (s.resolution) {
        rows.push({ status: 'applied', kind: 'llm', note: `Location shows them on site until ${fmtT(s.geo[1])}, so we suggest a clock-out at ${fmtT(s.resolution.out)}. Worth a look.`, chips: ['Timesheet', `Geofence exit ${fmtT(s.geo[1])}`] });
        rows.push({ status: 'applied', kind: 'human', note: `${s.resolution.by} confirmed: ${s.resolution.quote}. Clock-out set to ${fmtT(s.resolution.out)}.` });
      } else rows.push({ status: 'held', note: 'Pay is on hold until the manager confirms the clock-out', effect: { holdAll: true } });
      return rows;
    },
  },
  {
    id: 'CS-16H', bucket: 'Common sense', kind: 'det',
    sentence: 'A single time entry over the length cap is presumed a missed clock-out, not real time',
    ...
    evaluate(s, ctx, P) {
      ...
      if (m > MIN(P('CS-16H', 'cap_h'))) return [{ status: 'held', note: `Time entry shows ${fmtHM(m)}, longer than the ${P('CS-16H', 'cap_h')}h limit. Probably a missed clock-out, so pay is on hold.`, effect: { holdAll: true } }];
  ...
  },
  {
    ...
    evaluate(s, ctx) {
      ...
      if (!o) return [{ status: 'pass', note: 'No overlapping time entry' }];
      return [{ status: 'held', note: `Clocked in at ${o.fac.name} at the same time. Both are on hold until we know which is right.`, effect: { holdAll: true } }];
    },
  },
  {
    ...
    evaluate(s, ctx, P) {
      ...
      if (v > P('CS-SPEED', 'max_mph')) return [{ status: 'flag', note: isFinite(v) ? `Would have had to travel ${Math.round(v)} mph from the last site. One of the punches is wrong.` : 'Clocked in at two sites at the same moment. One of the punches is wrong.' }];
  ...
  },
  {
    ...
    evaluate(s, ctx, P) {
      ...
      if (n >= P('CS-EXACT', 'streak')) return [{ status: 'flag', note: `${n} days in a row with exactly the same hours. Probably typed in by hand, so check the source.` }];
      return [{ status: 'pass', note: 'Duration pattern normal' }];
    },
  },
  {
    id: 'CS-EDIT', bucket: 'Common sense', kind: 'det',
    sentence: 'Any time entry edit after supervisor approval re-triggers review',
    source: { doc: 'Wage-theft litigation pattern', cite: 'Post-approval edits are the classic vector' },
    scope: () => true, params: {},
    evaluate(s) {
      if (s.editedAfterApproval) return [{ status: 'flag', note: `${s.editedAfterApproval} changed this after it was approved. It needs another look.` }];
      return [{ status: 'pass', note: 'No post-approval edits' }];
    },
  },
  {
    id: 'SRC-VMS-01', bucket: 'Common sense', kind: 'det',
    sentence: 'Hours in the ATS must match the hours the client approved in their own system',
    source: { doc: 'Ops heuristic', cite: 'Pay comes from one system, the invoice from the other' },
    scope: s => s.vms != null,
    params: { tolerance_min: p(15, 0, 60, 5, 'min', 'Allowed difference') },
    evaluate(s, ctx, P) {
      const w = ctx.workedMin(s), gap = Math.abs(w - s.vms.min);
      if (gap <= P('SRC-VMS-01', 'tolerance_min')) return [{ status: 'pass', note: 'ATS and client hours agree' }];
      return [{ status: 'flag', note: `The ATS has ${fmtHM(w)}, but the client approved ${fmtHM(s.vms.min)} in their own timekeeping. Pay and the invoice won't match until one is corrected.` }];
    },
  },
  // Cross-source checks: they compare the ATS with a client's clock or rates, so they fire in the setup
    ...
    source: { doc: 'Ops heuristic', cite: 'Bill below loaded pay loses money every hour' }, scope: () => false, params: {}, evaluate: () => [] },
  {
    ...
    evaluate(s, ctx, P) {
      if (!s.geo) return [{ status: 'flag', note: 'No location record of them arriving on site' }];
      const pr = ctx.resolvedPairs(s); if (!pr.length) return [{ status: 'na', note: 'No punches' }];
      const gap = Math.abs(pr[0].in - s.geo[0]);
      if (gap > P('FAC-GEO-01', 'tol_min')) return [{ status: 'flag', note: `Clocked in ${gap} minutes away from when they arrived on site (we allow ${P('FAC-GEO-01', 'tol_min')})` }];
  ...
  },
  {
    ...
    evaluate(s, ctx, P) {
      ...
      if (gap > P('FAC-BADGE-01', 'tol_min')) {
        return [
          { status: 'flag', note: `Clocked out at ${fmtT(pr[pr.length - 1].rawOut)} but badged out at ${fmtT(s.badgeOut)}, ${gap} minutes later` },
          s.resolution
            ? { status: 'applied', kind: 'human', note: `${s.resolution.by} confirmed: ${s.resolution.quote}. Paid until the badge-out.`, effect: { extendToMin: s.badgeOut } }
            : { status: 'held', note: 'Waiting for the site lead to confirm', effect: { holdAll: true } },
        ];
      }
  ...
  },
  {
    ...
    evaluate(s, ctx) {
      ...
      if (s.mealEvidence === false) return [
        ...
        { status: 'applied', kind: 'llm', note: 'No break shows up in their movement, so we put the 30 minutes back. Worth a look.', effect: { premiumMin: 30 }, chips: ['Location trace', 'No meal punch'] },
      ];
  ...
  },
  {
    ...
    evaluate() { return [{ status: 'applied', note: 'Paid to the exact minute, no rounding' }]; },
  },
  {
    ...
    evaluate(s, ctx, P) { return [{ status: 'applied', note: `Clock times rounded to the nearest ${P('TB-ROUND-7', 'inc_min')} minutes` }]; },
  },
  {
    ...
    evaluate(s, ctx, P) {
      ...
      if (!s.meal) {
        ...
        if (worked <= dl) return [{ status: 'pass', note: 'Day shorter than the meal deadline' }];
        return [{ status: 'flag', note: `No meal break in a ${fmtHM(worked)} day, so they're owed an extra hour of pay`, effect: { premiumHours: 1 } }];
      }
      const off = ctx.resolvedPairs(s).length ? s.meal[0] - ctx.resolvedPairs(s)[0].in : null;
      const len = s.meal[1] - s.meal[0];
      if (off != null && off > dl) return [{ status: 'flag', note: `Meal break started ${fmtHM(off)} in, after the ${P('CA-MB-01', 'deadline_h')}-hour deadline, so they're owed an extra hour of pay`, effect: { premiumHours: 1 } }];
      if (len < P('CA-MB-01', 'min_len')) return [{ status: 'flag', note: `Meal break was only ${len} minutes (needs ${P('CA-MB-01', 'min_len')}), so they're owed an extra hour of pay`, effect: { premiumHours: 1 } }];
  ...
  },
  {
    ...
    evaluate(s, ctx, P) {
      ...
      return [{ status: 'applied', note: `Worked past ${P('CA-OT-8', 'daily_h')} hours: ${fmtHM(ot)} at time and a half${dt ? ` and ${fmtHM(dt)} at double time` : ''}`, effect: { otPremiumMin: ot * 0.5 + dt * 1.0, dailyOtMin: ot + dt } }];
    },
  },
  {
    id: 'FED-OT-40', bucket: 'Legal', kind: 'det',
    sentence: 'Over 40 hours in the workweek is 1.5× the regular rate',
    source: { doc: 'FLSA · 29 CFR 778', cite: 'Workweek = fixed Mon to Sun here' },
    scope: () => true,
    ...
    evaluate(s, ctx, P) {
      ...
      if (arr[arr.length - 1] !== s) return [{ status: 'na', note: 'Assessed on the last time entry of the week' }];
      ...
      return [{ status: 'applied', note: `Worked past 40 hours this week: ${fmtHM(over)} at time and a half`, effect: { otPremiumMin: over * 0.5 } }];
    },
  },
  {
    id: 'FED-RR-01', bucket: 'Legal', kind: 'det',
    sentence: 'Overtime is 1.5× the regular rate, and the regular rate includes night and weekend differentials',
    source: { doc: 'FLSA · 29 CFR 778.207', cite: 'Differentials belong in the regular rate' },
    scope: s => !!s.diff,
    params: {},
    evaluate(s, ctx, P) {
      const arr = ctx.byWorker.get(s.worker) || [];
      if (arr[arr.length - 1] !== s) return [{ status: 'na', note: 'Assessed on the last time entry of the week' }];
      const daily = arr.reduce((t, x) => t + (x._dailyOtMin || 0), 0);
      const over = ctx.weeklyWorked(s.worker) - MIN(P('FED-OT-40', 'weekly_h')) - daily;
      if (over <= 0) return [{ status: 'pass', note: 'No weekly overtime' }];
      const owed = Math.round(H(over) * 1.5 * s.diff * 100) / 100;
      return [{ status: 'flag', note: `Overtime was paid at 1.5× the base rate, leaving out the night differential. The differential is part of the regular rate, so ${fmtHM(over)} of overtime was underpaid.`, effect: { premiumAmt: owed } }];
    },
  },
  {
    id: 'CA-SS-01', bucket: 'State', kind: 'det',
    sentence: 'CA: an unpaid gap over an hour splitting the day owes a 1-hour premium at minimum wage, offset by earnings above minimum',
    source: { doc: 'IWC orders §4(C)', cite: 'Split-day premium with offset' },
    scope: s => s.fac.state === 'CA',
    ...
    evaluate(s, ctx, P) {
      ...
      return [{ status: 'flag', note: `Split day with a ${fmtHM(gap)} gap, so they're owed a ${money(prem)} premium`, effect: { premiumAmt: prem } }];
    },
  },
  {
    id: 'CA-RT-01', bucket: 'State', kind: 'det',
    sentence: 'CA: sent home before half the scheduled hours → pay half the schedule (min 2h, max 4h)',
    ...
    evaluate(s, ctx, P) {
      ...
      return [{ status: 'flag', note: `Scheduled for ${fmtHM(sm)} but only worked ${fmtHM(w)}. They're owed at least ${fmtHM(owe)} of pay.`, effect: { topUpMin: top } }];
    },
  },
  {
    ...
    evaluate(s, ctx, P) {
      ...
      return [{ status: 'flag', note: `Start to finish was ${fmtHM(span)}, over ${P('NY-SOH-01', 'spread_h')} hours, so they're owed an extra hour at ${money(s.fac.minWage)}`, effect: { premiumAmt: s.fac.minWage } }];
    },
  },
  {
    id: 'REST-GAP-01', bucket: 'Local', kind: 'det',
    sentence: 'Fair-workweek cities: working inside the protected rest window between closing and opening pays a premium (gap threshold varies 9–11h by city)',
    ...
    evaluate(s, ctx, P) {
      ...
      if (g == null) return [{ status: 'na', note: 'No previous time entry' }];
      ...
      if (s.consentClopen) return [{ status: 'flag', note: `Only ${fmtHM(g)} between closing and opening. They agreed in writing, so they're owed a $100 premium.`, effect: { premiumAmt: 100 } }];
      return [{ status: 'flag', note: `Only ${fmtHM(g)} between closing and opening, and no written consent. They're owed a $100 premium and we need their consent.`, effect: { premiumAmt: 100 } }];
    },
  },
  {
    ...
    evaluate(s, ctx, P) {
      ...
      if (h < P('CHI-FWW-01', 'notice_days') * 24) return [{ status: 'flag', note: `Schedule changed ${Math.round(h)} hours before start, inside the ${P('CHI-FWW-01', 'notice_days')}-day notice window, so they're owed an extra hour of pay`, effect: { premiumHours: 1 } }];
      return [{ status: 'pass', note: 'Change outside notice window' }];
    },
  },
  {
    id: 'CON-MIN-4H', bucket: 'Contract', kind: 'det',
    sentence: 'A worker who reports as scheduled is paid (and billed) a minimum of 4 hours, even if sent home',
    ...
    evaluate(s, ctx, P) {
      ...
      return [{ status: 'flag', note: `Only worked ${fmtHM(w)}. The contract guarantees ${P('CON-MIN-4H', 'min_h')} hours, so we pay the difference.`, effect: { topUpMin: m - w } }];
    },
  },
  {
    id: 'CON-SUTTER-01', bucket: 'Contract', kind: 'det',
    sentence: 'Pacific Cold Storage orientation hours pay the training rate and bill $0',
    source: { doc: 'Pacific Cold Storage MSA §4.2', cite: 'Orientation carve-out' },
    scope: s => s.fac === FACILITIES.sutter,
    params: { training_rate: p(18, 15, 26, 0.5, '$/h', 'Training rate') },
    evaluate(s, ctx, P) {
      if (!s.orientation) return [{ status: 'pass', note: 'Not orientation' }];
      if (s.rate !== P('CON-SUTTER-01', 'training_rate')) return [{ status: 'flag', note: `Orientation was paid at the normal ${money(s.rate)}/h. It should be the training rate of ${money(P('CON-SUTTER-01', 'training_rate'))}/h, and it isn't billed to the client.`, effect: { rateOverride: P('CON-SUTTER-01', 'training_rate') } }];
  ...
  },
  {
    ...
    evaluate(s, ctx, P) {
      ...
      if (arr[arr.length - 1] !== s) return [{ status: 'na', note: 'Assessed on the last Mercy time entry of the week' }];
      ...
      return [{ status: 'held', note: `${fmtHM(over)} over Mercy's weekly hour limit. That time is on hold and Dana K. has been asked.`, effect: { holdMin: over } }];
    },
  },
  {
    id: 'VER-HC-01', bucket: 'Vertical', kind: 'det',
    sentence: 'Healthcare: handoff overlap up to 15 minutes is paid, expected time',
    source: { doc: 'Vertical pack · healthcare', cite: 'Handoff overlap' },
    scope: s => s.fac.vertical === 'healthcare', params: {},
    evaluate() { return [{ status: 'applied', note: 'Up to 15 minutes of handover counted as work time' }]; },
  },
  {
    ...
    evaluate(s, ctx, P) {
      ...
      return [{ status: 'applied', kind: 'llm', note: `Clocked out ${past} minutes after the event ended. Location shows them leaving with the crowd, which looks fine, but it's longer than usual. Worth a look.`, chips: ['Location trace', 'Schedule'] }];
    },
  },
];
...
function runEngine(week, paramOverrides) {
  ...
  for (const s of week) {
    const rows = [];
    if (s._clean) { s._dailyOtMin = 0; results.set(s.id, rows); continue; }
    for (const r of RULES) {
      if (r.id === 'FED-OT-40' || r.id === 'FED-RR-01' || r.id === 'TW-1187') continue; // pass 2
  ...
  for (const s of week) {
    ...
    if (s._clean) continue;
    for (const rid of ['FED-OT-40', 'FED-RR-01', 'TW-1187']) {
      const r = RULES.find(x => x.id === rid);
      if (!r.scope(s)) continue;
...
function makeWeek({ seed = 20260824, scripted = true, start } = {}) {
  ...
  const crews = [
    ...
    { worker: 'Priya S.', fac: F.sutter, role: 'Warehouse associate', rate: 26.00, days: [1, 2, 3, 4], start: T(7, 0), len: 480 },
    ...
    { worker: 'Grace H.', fac: F.stmarks, role: 'Banquet server', rate: 23.00, days: [0, 1, 2, 3], start: T(8, 30), len: 480 },
    { worker: 'Sam O.', fac: F.mercy, role: 'Housekeeping', rate: 24.50, days: [5, 6], start: T(8, 0), len: 450 },
    { worker: 'Nina T.', fac: F.sutter, role: 'Night loader', rate: 26.00, days: [0, 1, 2], start: T(19, 0), len: 480 },
  ...
  if (scripted) {
    ...
    m4833.resolution = { by: 'Ray P. · site lead', quote: 'Yeah the clock maxes out, he was here till quarter to five' };
    ...
    // Grace Mon: overlapping Harbor Point + Mercy pickup (CS-OVLP + CS-SPEED)
    ...
    }); // overlaps 8:30-16:30 Harbor Point shift → CS-OVLP flags both

    // 4842: Priya Thu — Bullhorn has 8h, the client's UKG approved 7h 30m (SRC-VMS-01)
    const m4842 = W.find(s => s.worker === 'Priya S.' && s.day === 3);
    m4842.id = '4842'; m4842.punches = [{ in: T(6, 58), out: T(15, 28) }]; m4842.meal = [T(11, 0), T(11, 30)]; m4842.vms = { min: 450 };

    // 4844: Priya Wed — one punch pair, no meal, 8h 31m in California (CA-MB-01)
    const m4844 = W.find(s => s.worker === 'Priya S.' && s.day === 2);
    m4844.id = '4844'; m4844.punches = [{ in: T(6, 57), out: T(15, 28) }]; m4844.meal = null;

    // 4843: Reggie picks up Saturday; overtime paid without his $1.50 shift differential (FED-RR-01)
    for (const [d, inM, outM] of [[5, T(6, 2), T(14, 27)]]) shift({
      worker: 'Reggie F.', fac: F.bayview, role: 'Picker', rate: 17.50, day: d, sched: [T(6, 0), T(14, 30)],
      punches: [{ in: inM, out: outM }], badgeIn: inM - 3, badgeOut: outM + 2, meal: [T(10, 0), T(10, 30)],
    });
    const reggie = W.filter(s => s.worker === 'Reggie F.');
    for (const s of reggie) s.diff = 1.5;
    reggie[reggie.length - 1].id = '4843';
  }

  // The rest of the agency's roster, in every week: 2,000-odd workers, a few more or fewer each week, on three clean days each (under 8h, early
  // punched meal). Marked _clean so the engine skips them; checking entries built to pass adds nothing.
  const FIRST = ['Ana', 'Ben', 'Carla', 'Diego', 'Erin', 'Femi', 'Gabe', 'Hana', 'Ivan', 'Jade', 'Kofi', 'Lena', 'Malik', 'Nora', 'Oscar', 'Paula',
    'Quinn', 'Rosa', 'Sid', 'Tara', 'Uma', 'Victor', 'Wen', 'Xavi', 'Yara', 'Zane', 'Abel', 'Bea', 'Cruz', 'Dina', 'Eli', 'Faye', 'Gus', 'Iris', 'Jon', 'Kira', 'Leo', 'Mei', 'Ned', 'Ola'];
  const LAST = ['Adams', 'Baker', 'Castro', 'Dunn', 'Ellis', 'Ford', 'Grant', 'Hayes', 'Ingram', 'James', 'Khan', 'Lowe', 'Mills', 'Nash', 'Owens', 'Price',
    'Quint', 'Reyes', 'Shaw', 'Tran', 'Upton', 'Vega', 'Wade', 'Xu', 'Young', 'Zamora', 'Arias', 'Bell', 'Cole', 'Diaz', 'Estes', 'Fox', 'Gill', 'Hart', 'Irwin',
    'Joyce', 'Kerr', 'Lamb', 'Moss', 'Noble', 'Oliver', 'Pace', 'Rhodes', 'Stone', 'Tate', 'Usher', 'Vance', 'Webb', 'York', 'Zeller',
    'Avila', 'Burke', 'Chung', 'Doyle', 'Ekwueme', 'Flynn', 'Gray', 'Hull', 'Imani', 'Jordan'];
  const roster = [[F.sutter, 'Warehouse associate', T(6, 0), 24.00, 400], [F.lonestar, 'Picker', T(7, 0), 17.50, 500 + Number(weekKey) % 53], [F.bayview, 'Picker', T(7, 0), 17.50, 300],
    [F.northbank, 'Event staff', T(14, 0), 20.00, 200], [F.wicker, 'Prep cook', T(9, 0), 19.75, 200], [F.stmarks, 'Room attendant', T(8, 0), 21.00, 200], [F.mercy, 'Environmental services', T(7, 0), 24.50, 203]];
  let n = 0;
  for (const [fac, role, start, rate, count] of roster) for (let i = 0; i < count; i++, n++) {
    const worker = `${FIRST[n % FIRST.length]} ${LAST[Math.floor(n / FIRST.length) % LAST.length]}`;
    for (const d of [[0, 1, 2], [2, 3, 4], [0, 2, 4]][n % 3]) {
      const inM = start + (n * 7 + d * 3) % 9 - 4;
      let out = inM + 465 + d * 2 + n % 5 + 32, meal = [inM + 195 + (n + d * 7) % 31, inM + 225 + (n + d * 7) % 31 + n % 4], clean = true;
      // A few percent of California entries go wrong, differently each week: long days the agent pays as daily
      // overtime, and missing meal punches a person has to review.
      const k = n * 5 + d + Number(weekKey) % 97;
      if (fac.state === 'CA' && !fac.autoDeduct && k % (8 + Number(weekKey) % 3) === 0) { out += 70; clean = false; }
      else if (fac.state === 'CA' && !fac.autoDeduct && k % (26 + Number(weekKey) % 7) === 0) { meal = null; out -= 30; clean = false; }
      shift({ worker, fac, role, rate, day: d, sched: [start, start + 510], punches: [{ in: inM, out }], meal,
        geo: fac.geofence ? [inM - 5, out + 4] : null, badgeIn: fac.badge ? inM - 3 : null, badgeOut: fac.badge ? out + 2 : null, _clean: clean });
    }
  }
...
export function selfCheck() {
  ...
  const lines = [];
  const assert = (cond, msg) => { if (!cond) throw new Error('FAIL: ' + msg); lines.push('ok: ' + msg); };
  ...
  lines.push(`ok: week: ${week.length} shifts · flags ${totals.flags} · held ${totals.held} · under ${money(totals.under)} · over ${money(totals.over)} · CA-MB-01 fires ${bt1.fires}`);
  return lines;
}
