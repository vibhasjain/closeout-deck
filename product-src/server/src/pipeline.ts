...
export interface SourceInput { id: string; set: 1 | 2 | 3; system: string; site?: string | null; method: 'upload' | 'simulated'; lastReceivedAt?: string | null }
...
export interface CyclePayload {
  ...
  week: (Omit<Shift, 'fac'> & { fac: number; prov: Prov & { hoursOnly?: boolean }; entryIds: string[] })[]
  ...
  intake: { sources: { id: string; name: string; short: string; set: 1 | 2 | 3; method: string; lastReceived: string | null }[]; expected: { worker: string; client: string; day: number; source: string; onSite?: number }[]; received: string[] }
}
export interface PipelineInput { email: string; cycle: Cycle; calendar?: Calendar; entries: TimeEntry[]; files: FileInput[]; facts: FactInput[]; sources?: SourceInput[]; engineSha?: string; now?: Date; dismissed?: string[] }
...
export function assemble(input: PipelineInput): { shifts: BuiltShift[]; sites: Site[]; gaps: Gap[]; entries: TimeEntry[]; workweekStart: number } {
  ...
  for (const groupKey of new Set([...sets[0].keys(), ...sets[1].keys()])) {
    ...
    if (!siteFact && inCycle) gap('site', any.siteKey, any, ['State', 'Local'], `Where is ${any.site}? City and state is enough.`)
...
export function pipelineInputHash(input: Pick<PipelineInput, 'cycle' | 'calendar' | 'files' | 'facts' | 'engineSha' | 'dismissed'>): string {
  return digest(canonical({ engine: input.engineSha ?? engineSha, cycle: [isoDate(input.cycle.start), isoDate(input.cycle.end), input.cycle.status], calendar: input.calendar ?? calendarFrom(), files: input.files.filter(f => !f.status || f.status === 'normalized').map(f => [f.id, f.sha256, f.mappingId, f.normalizedAt, f.periodEnd]).sort((a, b) => String(a[0]).localeCompare(String(b[0]))), facts: [...input.facts].sort((a, b) => `${a.kind}|${a.key}`.localeCompare(`${b.kind}|${b.key}`)), dismissed: [...(input.dismissed ?? [])].sort(), overrides: {} }))
}
...
export function buildCycle(input: PipelineInput): { payload: CyclePayload; findings: FindingCase[]; inputHash: string } {
  ...
  for (const [week, bs] of weeks) {
    ...
    for (let i = 0; i < bs.length; i++) {
      ...
      kept.push({ built: b, result: { ...result, shift: { ...result.shift, worker: b.shift.worker, day: days(b.workDate, start) } }, worked: run.ctx.workedMin(result.shift), weekWorked: run.ctx.weeklyWorked(result.shift.worker) })
    }
  }
