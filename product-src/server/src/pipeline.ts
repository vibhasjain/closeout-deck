...
/** Labels that name a kind of data, not the system it came from. */
export const GENERIC_SYSTEM = /^\s*(time entries|time entry|timesheets?|time export|spreadsheet|upload|sample)\s*$/i
const extraSummary = (n: number, moved: number) =>
  `${n.toLocaleString('en-US')} time ${n === 1 ? 'entry' : 'entries'}${moved ? `, ${moved.toLocaleString('en-US')} moved into this week` : ''}`
...
function templateGroup(ruleId: string, list: Detail[], input: PipelineInput, rates: boolean): FindingGroup {
  ...
  // A generic or shared label ("Time entries") would make one source read as both sides of a difference.
  const named = (set: 1 | 2) => input.sources?.find(s => s.set === set && sourceIds.has(s.id))?.system
  const own = named(1), clock = named(2), distinct = !!own && !!clock && own.toLowerCase() !== clock.toLowerCase()
  const ownSystem = own && !GENERIC_SYSTEM.test(own) && (distinct || !clock) ? own : 'the agency export'
  const clockSystem = clock && !GENERIC_SYSTEM.test(clock) && (distinct || !own) ? clock : 'the client clock'
...
export function buildCycle(input: PipelineInput): { payload: CyclePayload; findings: FindingCase[]; inputHash: string } {
  ...
  const extraGroups = extraRules.map((ruleId): FindingGroup => { const list = details.filter(d => !d.primary && d.row.ruleId === ruleId), rule = RULES.find(r => r.id === ruleId), amount = cents(total(list, d => Math.abs(d.row.exposure ?? d.row.delta))); return { ruleId, tag: ruleId, title: rule?.sentence ?? ruleId, summary: extraSummary(list.length, list.filter(d => d.built.wrongWeek).length), why: rule?.source.doc ?? '', hoursLabel: fmtHM(total(list, d => d.minutes)), amount, amountLabel: `$${amount.toFixed(2)}`, action: 'Check the evidence on each time entry before approving', deadline: 'payroll', sources: [], dispute: 'Worker dispute', cases: list.length } })
