...
Eighteen slides, all illustrated storyboard frames in one language. No UI screenshots, no close slide. Narrative: tech-enabled staffing firms deploy AI faster; AI has done recruiting and matching, the middle office (pay and bill) is next; workers and customers both re-choose you every day, so pay accuracy is revenue protection (frames 01–12). Then the product story in six beats (slides 13–18): data auto-ingests with a digital worker in a virtual browser (real vendor tiles from `logos/tiles/` overlaid), location tracked in real time, paper timesheets and clock-ins by SMS and phone call, payroll rules applied (flag → clause → compiled rule), discrepancies mediated agentically, then uploaded to payroll agentically through the browser. 
