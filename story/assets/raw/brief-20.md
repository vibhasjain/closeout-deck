...
Read story/STORYBOARD.md. Build the prompt as: the full STYLE block verbatim, a blank line, then the SCENE paragraph for frame "20 · digital-worker" verbatim. Where the SCENE explicitly allows text (frame 21's chalkboard), that allowance overrides the STYLE block's no-text rule for that element only; say so in the prompt.

1. Call the image generation tool with that prompt (landscape 16:9, highest quality available).
2. The tool saves a PNG under ~/.codex/generated_images/<thread-id>/. Copy it to story/assets/raw/20-digital-worker.png.
...
4. Append one line to story/assets/raw/manifest.md: `20-digital-worker.png | <original path> | <retry note or none>`.

Retry transient errors. Finish with the line FRAME 20 DONE.
