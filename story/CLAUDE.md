...
Read this before touching anything in `story/`. It is the full context for extending or editing the deck: what it is, the illustration language, how a slide is built, how frames get generated, and the rules the owner has laid down in review. Live at https://closeoutcopilot.com/story/ (Netlify deploys on push to `main`; `story` and `logos` are on the whitelist in `scripts/assemble-dist.sh`, which ships everything here except `assets/raw/` and `*.md`). The Agent Keyboard bar on the deck is `data-site="closeout"` (this repo, page session `/story`); guests provisioned for it are confined to `story/`.
...
2. Generate the image with the Codex CLI's built-in image generation: one `codex exec` session per frame, in parallel, effort ultra; confirm `reasoning effort: ultra` in the log header before trusting the run:
   ...
   `codex` is installed on the Agent Keyboard server too (its `~/.codex/config.toml` is seeded with gpt-6-astra / ultra / fast / full access, the ChatGPT login lives on the volume at `/data/.codex/auth.json`), so run the command there exactly as above, from the checkout root (`<repo root>` = your cwd). Verified on the server 2026-09-25: `-s workspace-write` works (a bubblewrap warning is harmless) and `compress.sh` uses the preinstalled Pillow (no `magick`/`sips` there). One frame takes 2–4 minutes, longer than a default shell timeout: start each `codex exec` in the background (or with a 10-minute timeout) and wait for `FRAME NN DONE`. Run several frames as parallel sessions, never one long session. If `codex login status` says you are not logged in, stop and tell the owner to run `fly ssh console -a agent-keyboard` then `HOME=/data codex login --device-auth`; never invent credentials. Elsewhere, if `magick` and `sips` are both missing, `compress.sh` falls back to Pillow: `pip3 install --user pillow` once.
